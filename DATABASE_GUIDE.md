...
This guide explains how to manage your database schema using Prisma with Supabase.
...
To pull the current schema from your Supabase database:
...
### Starting Fresh with Existing Supabase Database
...
Prisma doesn't manage RLS policies. Apply them separately in Supabase:

```sql
-- Example: Enable RLS on contacts table
ALTER TABLE contacts ENABLE ROW LEVEL SECURITY;

-- Create policies in Supabase dashboard or via SQL
```

### 2. Supabase Auth Integration

The `contacts` table references Supabase auth users via `supabase_user_id`. This is a soft reference since Prisma can't directly reference `auth.users`.

### 3. Existing Tables

If you have existing Supabase tables (like `profiles`, `links`, etc.), the introspection will include them. You can:
...
Supabase triggers and functions aren't managed by Prisma. Create them separately:
...
# 2. Pull current schema from Supabase
npm run prisma:introspect

# 3. Review the generated schema.prisma
# - Remove Supabase internal tables
...
SUPABASE_PROJECT_ID=okzhikykpwubsrhuwbto npm run db:types
