...
import { retired-providerAdmin } from '../config/retired-provider.js';
...
router.post('/lemon', async (req: Request, res: Response) => {
  try {
    ...
    const { error: auditError } = await retired-providerAdmin
    ...
    const { data: existing } = await retired-providerAdmin
      .from('subscriptions')
      .select('id')
